...
class TestCourseView(TestCase):
    ...
    def test_cannot_create_courses_with_the_same_name(self):
        ...
        self.assertDictEqual(
            course_1_again.json(), {"error": "Course with this name already exists"}
        )
    ...
    def test_update_course_name(self):
        ...
        response = self.client.put("/api/courses/1/", self.course_data2, format="json")
    ...
    def test_cannot_update_course_to_an_existing_name(self):
        ...
        # Criação do curso2
        self.client.post("/api/courses/", self.course_data2, format="json")

        # Altera nome do curso2 para curso1
        response = self.client.put("/api/courses/2/", self.course_data, format="json")

        # Verifica se deu erro na hora de atualizar o curso
        self.assertEqual(response.status_code, 400)
        self.assertDictEqual(
            response.json(), {"error": "Course with this name already exists"}
        )
    ...
    def test_enroll_students_with_invalid_id(self):
        ...
        self.assertIn(
            ...
            ["invalid user_id list", "Only students can be enrolled in the course."],
        )
        self.assertEqual(response.status_code, 400)
...
class TestActivityView(TestCase):
    ...
    def test_facilitator_or_instructor_can_update_an_activity(self):
        ...
        self.assertDictEqual(
            activity.json(),
            {"id": 1, "title": "activity4", "points": 8, "submissions": []},
        )
        ...
        self.assertDictEqual(
            activity.json(),
            {"id": 1, "title": "activity1", "points": 5, "submissions": []},
        )
        self.assertEqual(activity.status_code, 200)
    ...
    def test_if_it_is_not_possible_to_create_activities_with_the_same_title(self):
        ...
        self.assertDictEqual(
            activity2.json(), {"error": "Activity with this name already exists"}
        )
    ...
    def test_if_it_is_not_possible_to_update_activity_to_an_existing_title(self):
        ...
        # Criação da activity 1 pelo instructor
        self.client.post("/api/activities/", self.activity_data_1, format="json")

        # Criação da activity 2 pelo instructor
        self.client.post("/api/activities/", self.activity_data_2, format="json")
        ...
        self.assertDictEqual(
            activity.json(), {"error": "Activity with this name already exists"}
        )
